...
If the value of the state property from the slot's `_condition` value is truthy, elements with `if="true"` will render, and if its falsy - elements with `_if="false"` will render.
